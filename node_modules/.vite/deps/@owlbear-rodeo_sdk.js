...
var SceneItemsApi = class {
  ...
  updateItems(filterOrItems, update, updateAttachments = true) {
    return __awaiter7(this, void 0, void 0, function* () {
      let items;
...
var SceneLocalApi = class {
  ...
  updateItems(filterOrItems, update, fastUpdate, updateAttachments = true) {
    return __awaiter8(this, void 0, void 0, function* () {
      let items;
      if (this.isItemArray(filterOrItems)) {
        items = filterOrItems;
...
var InteractionApi = class {
  ...
  startItemInteraction(baseState, updateAttachments = true) {
    return __awaiter15(this, void 0, void 0, function* () {
      const { id } = yield this.messageBus.sendAsync("OBR_INTERACTION_START_ITEM_INTERACTION", { baseState, updateAttachments });
      let prev = baseState;
...
function roundTo(x2, to) {
  return Math.round(x2 / to) * to;
}
function floorTo(x2, to) {
  return Math.floor(x2 / to) * to;
}
...
var Math2 = class {
   ...
   */
  static magnitude(p2) {
    const dist = Math.sqrt(this.magnitudeSquared(p2));
    if (isNaN(dist)) {
      return 0;
    }
    return dist;
   ...
   */
  static normalize(p2) {
    const l2 = this.magnitude(p2);
    if (l2 === 0) {
      return { x: 0, y: 0 };
    }
    return this.divide(p2, l2);
  }
  /**
  ...
  static min(a2, b2) {
    if (typeof b2 === "number") {
      return { x: Math.min(a2.x, b2), y: Math.min(a2.y, b2) };
    } else {
  ...
  static max(a2, b2) {
    if (typeof b2 === "number") {
      return { x: Math.max(a2.x, b2), y: Math.max(a2.y, b2) };
    } else {
   ...
   */
  static componentMax(a2) {
    return a2.x > a2.y ? a2.x : a2.y;
  }
  /**
   * Rounds `p` to the nearest value of `to`
   */
  static roundTo(p2, to) {
    return {
      x: roundTo(p2.x, to.x),
      y: roundTo(p2.y, to.y)
    };
   ...
   */
  static sign(a2) {
    return { x: Math.sign(a2.x), y: Math.sign(a2.y) };
  }
  /**
   * @returns The component wise absolute of `a`
   */
  static abs(a2) {
    return { x: Math.abs(a2.x), y: Math.abs(a2.y) };
  }
  /**
  ...
  static pow(a2, b2) {
    if (typeof b2 === "number") {
      return { x: Math.pow(a2.x, b2), y: Math.pow(a2.y, b2) };
    } else {
      return { x: Math.pow(a2.x, b2.x), y: Math.pow(a2.y, b2.y) };
    }
  }
  /**
   * Clamps `a` between `min` and `max`
   */
  static clamp(a2, min, max) {
    return {
      x: Math.min(Math.max(a2.x, min), max),
      y: Math.min(Math.max(a2.y, min), max)
    };
  }
  /**
  ...
  static pointInPolygon(p2, points) {
    ...
    if (p2.x < bounds.min.x || p2.x > bounds.max.x || p2.y < bounds.min.y || p2.y > bounds.max.y) {
      return false;
    }
    let isInside = false;
    for (let i2 = 0, j2 = points.length - 1; i2 < points.length; j2 = i2++) {
      const a2 = points[i2].y > p2.y;
      const b2 = points[j2].y > p2.y;
      if (a2 !== b2 && p2.x < (points[j2].x - points[i2].x) * (p2.y - points[i2].y) / (points[j2].y - points[i2].y) + points[i2].x) {
        isInside = !isInside;
      }
    }
    return isInside;
   ...
   */
  static compare(a2, b2, threshold) {
    return this.magnitudeSquared(this.subtract(a2, b2)) < threshold * threshold;
  }
  /**
   * @returns The distance between two vectors
   */
  static distance(a2, b2) {
    return this.magnitude(this.subtract(a2, b2));
  }
  /**
   * Linear interpolate between `a` and `b` by `alpha`
   */
  static lerp(a2, b2, alpha) {
    return { x: lerp(a2.x, b2.x, alpha), y: lerp(a2.y, b2.y, alpha) };
  }
  /**
...
var MathM = class _MathM {
  ...
  static multiply(a2, b2) {
    return [
      a2[0] * b2[0] + a2[1] * b2[3] + a2[2] * b2[6],
      a2[0] * b2[1] + a2[1] * b2[4] + a2[2] * b2[7],
      a2[0] * b2[2] + a2[1] * b2[5] + a2[2] * b2[8],
      a2[3] * b2[0] + a2[4] * b2[3] + a2[5] * b2[6],
      a2[3] * b2[1] + a2[4] * b2[4] + a2[5] * b2[7],
      a2[3] * b2[2] + a2[4] * b2[5] + a2[5] * b2[8],
      a2[6] * b2[0] + a2[7] * b2[3] + a2[8] * b2[6],
      a2[6] * b2[1] + a2[7] * b2[4] + a2[8] * b2[7],
      a2[6] * b2[2] + a2[7] * b2[5] + a2[8] * b2[8]
    ];
  }
  /**
  ...
  static fromRotation(rotation) {
    const r2 = toRadians(rotation);
    return [Math.cos(r2), -Math.sin(r2), 0, Math.sin(r2), Math.cos(r2), 0, 0, 0, 1];
  }
  /**
  ...
  static fromItem(item) {
    const t2 = _MathM.fromPosition(item.position);
    const s2 = _MathM.fromScale(item.scale);
    const r2 = _MathM.fromRotation(item.rotation);
    return _MathM.multiply(_MathM.multiply(t2, r2), s2);
  }
  /**
  ...
  static decompose(matrix) {
    ...
    if (m1 != 0 || m4 != 0) {
      var r2 = Math.sqrt(m1 * m1 + m4 * m4);
      result.rotation = m4 > 0 ? Math.acos(m1 / r2) : -Math.acos(m1 / r2);
      result.scale = { x: r2, y: delta / r2 };
    } else if (m2 != 0 || m5 != 0) {
      var s2 = Math.sqrt(m2 * m2 + m5 * m5);
      result.rotation = Math.PI / 2 - (m5 > 0 ? Math.acos(-m2 / s2) : -Math.acos(m2 / s2));
      result.scale = { x: delta / s2, y: s2 };
    } else {
    }
